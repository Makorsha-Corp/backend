"""Planned workers linked to workspace members for a work order."""
from sqlalchemy import Column, Integer, ForeignKey, UniqueConstraint
from sqlalchemy.orm import relationship

from app.db.base_class import Base


class WorkOrderAssignee(Base):
    __tablename__ = "work_order_assignees"
    __table_args__ = (
        UniqueConstraint("work_order_id", "user_id", name="uq_wo_assignee_wo_user"),
    )

    id = Column(Integer, primary_key=True, index=True, autoincrement=True)
    workspace_id = Column(Integer, ForeignKey("workspaces.id", ondelete="CASCADE"), nullable=False, index=True)
    work_order_id = Column(Integer, ForeignKey("work_orders.id", ondelete="CASCADE"), nullable=False, index=True)
    user_id = Column(Integer, ForeignKey("profiles.id", ondelete="CASCADE"), nullable=False, index=True)

    work_order = relationship("WorkOrder", back_populates="assignee_links")
    user = relationship("Profile", foreign_keys=[user_id])
