"""Transfer order event model - immutable activity log for a transfer order."""
...
class TransferOrderEvent(Base):
    """Append-only activity log for a transfer order."""

    __tablename__ = "transfer_order_events"

    id = Column(Integer, primary_key=True, index=True, autoincrement=True)
    workspace_id = Column(Integer, ForeignKey("workspaces.id", ondelete="CASCADE"), nullable=False, index=True)
    transfer_order_id = Column(
