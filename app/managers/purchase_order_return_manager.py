"""Purchase Order Return Manager - business logic for returning received PO goods"""
...
class PurchaseOrderReturnManager(BaseManager[PurchaseOrderReturn]):
    """Manager for purchase order return business logic."""

    def __init__(self):
        super().__init__(PurchaseOrderReturn)
        self.return_dao = purchase_order_return_dao
        self.return_item_dao = purchase_order_return_item_dao
        self.item_dao = purchase_order_item_dao

    def has_open_return(self, session: Session, purchase_order_id: int, workspace_id: int) -> bool:
        return bool(self.return_dao.get_open_by_purchase_order(
            session, purchase_order_id=purchase_order_id, workspace_id=workspace_id
        ))
    ...
    def validate_and_build_items(
        ...
        *,
        ...
        item_map = {
            it.id: it
            for it in self.item_dao.get_by_order(
                session, purchase_order_id=purchase_order_id, workspace_id=workspace_id
            )
        }
        built: List[dict] = []
        for line in data.items:
            po_line = item_map.get(line.po_item_id)
            if po_line is None:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f'Item {line.po_item_id} does not belong to this purchase order',
                )
            available = po_line.quantity_available_to_return
            if line.quantity_returned > available:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=(
                        f'Cannot return {line.quantity_returned} of line {po_line.line_number} — '
                        f'only {available} available to return'
                    ),
                )
            unit_price = line.unit_price if line.unit_price is not None else po_line.unit_price
            built.append({
    ...
    def get_return(
        self, session: Session, return_id: int, purchase_order_id: int, workspace_id: int
    ) -> PurchaseOrderReturn:
        ret = self.return_dao.get_by_id_and_workspace(session, id=return_id, workspace_id=workspace_id)
        if not ret or ret.purchase_order_id != purchase_order_id:
            raise HTTPException(
    ...
    def list_returns(
        self, session: Session, purchase_order_id: int, workspace_id: int
    ) -> List[PurchaseOrderReturn]:
        return self.return_dao.get_by_purchase_order(
            session, purchase_order_id=purchase_order_id, workspace_id=workspace_id
        )

    def sync_return_paid(self, session: Session, ret: PurchaseOrderReturn, workspace_id: int) -> bool:
...
purchase_order_return_manager = PurchaseOrderReturnManager()
