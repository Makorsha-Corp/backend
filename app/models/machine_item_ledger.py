"""Machine item ledger model - tracks all machine inventory movements"""
...
class MachineItemLedger(Base):
    """
    Ledger tracking all machine item transactions.
    Immutable audit trail of all inventory movements for machines.
    """

    __tablename__ = "machine_item_ledger"
    ...
    # Valid values: 'order', 'manual', 'adjustment', 'invoice_correction', 'transfer', 'consumption_event'
    ...
    transfer_destination_type = Column(String(50), nullable=True)  # 'storage', 'machine', 'damaged'
    transfer_destination_id = Column(Integer, nullable=True)  # factory_id, machine_id, etc.
    ...
    item = relationship("Item", backref="machine_ledger_entries")
    order = relationship("Order", backref="machine_ledger_entries")
    invoice = relationship("AccountInvoice", backref="machine_ledger_entries")
    performer = relationship("Profile", foreign_keys=[performed_by], backref="machine_ledger_entries")
