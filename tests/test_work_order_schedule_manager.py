...
def test_stage_day_factory_only_includes_unassigned_machines(
    ...
    mock_machine_dao.get_by_factory.return_value = [unassigned]
    mock_machine_dao.get_by_id_and_workspace.return_value = unassigned
    ...
    mock_machine_dao.get_by_factory.assert_called_once_with(
...
def test_stage_day_default_machine_without_section_does_not_error(
    ...
    assert len(created) == 1
    assert created[0].factory_section_id is None
    mock_machine_dao.get_by_factory.assert_not_called()
